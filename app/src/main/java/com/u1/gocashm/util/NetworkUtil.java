...
public class NetworkUtil {
    ...
    public static boolean isNetAvailable(Context context) {
        ...
        return isNetworkAvailableByPing();
    }
     ...
     * @return
     */
    public static boolean isNetworkAvailableByPing() {
        Runtime runtime = Runtime.getRuntime();
        try {
            Process p = runtime.exec("ping -c 1 -w 1 223.5.5.5");
            int ret = p.waitFor();
            return ret == 0;
        } catch (Exception e) {
    ...
    public static String getIp() {
        try {
            ...
            do {
                ...
                if (line.trim().matches("inet addr:(\\d{1,3}\\.){3}\\d{1,3}( ){2}" + "(Bcast:(\\d{1,3}\\.){3}\\d{1,3}( ){2}){0,1}" + "Mask:(\\d{1,3}\\.){3}\\d{1,3}")) {
                    ...
                    if (inetAddr != null && bcast != null && mask != null) {
                        return inetAddr;
                    }
                }
            }
